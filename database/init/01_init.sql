-- ============================================================
-- 01_init.sql - WPH Hospital Training System
-- รวม Schema + Migrations ทั้งหมดสำหรับ Docker Init
-- MySQL จะรัน script นี้อัตโนมัติตอน container แรกเปิด
-- ============================================================
...
-- ── 1. Create Database ─────────────────────────────────────
CREATE DATABASE IF NOT EXISTS wph_training_db
...
CREATE TABLE IF NOT EXISTS users (
    ...
    auth_method ENUM('local', 'provider_id') DEFAULT 'local',
...
-- ── 4. Provider-ID Registration Table ─────────────────────
...
SELECT
    m.media_id, m.filename, m.original_filename, m.file_type, m.file_size,
    m.mime_type, m.file_path, m.title, m.description, m.category, m.tags,
    m.view_count, m.download_count, m.upload_date, m.last_updated,
    d.department_name,
...
SELECT
    pr.registration_id, pr.provider_id, pr.cid, pr.full_name, pr.email,
    pr.position, pr.hospital, pr.status, pr.created_at, pr.rejection_reason,
    approver.full_name AS approved_by_name, pr.approved_at
...
SELECT
    u.user_id, u.employee_id, u.username, u.full_name, u.email,
    u.cid, u.position, u.hospital, u.provider_id, u.approval_status,
    u.is_active, approver.full_name AS approved_by_name,
